...
(function($) {
  ...
  let $body = $('body');
  let $style_switcher = $('#style-switcher');
  ...
  THEMEMASCOT.isLTR = {
	...
	function headerStyle() {
		if($('.main-header').length){
			...
			let siteHeader = $('.header-style-one');
	...
	const windowOn = $(window); // Define windowOn properly
  ...
  items.forEach(item => {
	...
	if ($('.banner-slider-one').length) {
		let swiper = new Swiper(".banner-slider-one", {
			slidesPerView: 1,
			spaceBetween: 0,
			speed: 600,
			loop: true,
...
if ($('.banner-two-slider-down').length) {
    let swiper = new Swiper(".banner-two-slider-down", {
        direction: "vertical",      
        slidesPerView: 3,           
        spaceBetween: 15,                
        loop: true,                 
...
if ($('.banner-two-slider-up').length) {
    let swiper = new Swiper(".banner-two-slider-up", {
        direction: "vertical",      
        slidesPerView: 3,           
        spaceBetween: 15,                
        loop: true,                 
	...
	if ($('.service-one-slider').length) {
		let swiper = new Swiper(".service-one-slider", {
			speed:1500,
			loop: true,
			slidesPerView: 1,
			spaceBetween: 0,
	...
	if ($('.service-two-slider').length) {
		let swiper = new Swiper(".service-two-slider", {
			speed: 1500,
			loop: true,
			autoplay: {
		    delay: 1500,
	...
	if ($('.project-one-slider').length) {
		let swiper = new Swiper(".project-one-slider", {
			speed: 500,
			loop: true,
			autoplay: {
		    delay: 1500,
	...
	if ($('.clients-swiper-one').length) {
		let swiper = new Swiper(".clients-swiper-one", {
			slidesPerView: 5,
			spaceBetween: 0,
			speed: 500,
			loop: true,
	...
	if ($('.testi-three-slider').length) {
		let swiper = new Swiper(".testi-three-slider", {
			slidesPerView: 1,
			spaceBetween: 0,
			speed: 600,
			loop: true,
	...
	if ($('.testimonial-two-slider').length) {
		let swiper = new Swiper(".testimonial-two-slider", {
			speed: 500,
			loop: true,
			autoplay: {
		    delay: 1500,
	...
	if ($('.testi-slider-content').length) {
		let swiper = new Swiper(".testi-slider-content", {
			speed:1500,
			loop: true,
			slidesPerView: 1,
			spaceBetween: 70,
    ...
    let $team_block = $('.team-block1 .inner-block');
  ...
  function show_secondary_price(pricing_tables){
    ...
    let pricing_btn = pricing_tables.find('.btn');
    let secondary_btn_url = pricing_btn.data("secondary-link");
    pricing_btn.attr("href", secondary_btn_url);
  }
  function hide_secondary_price(pricing_tables){
    pricing_tables.removeClass('show-secondary-price');
    let pricing_btn = pricing_tables.find('.btn');
    let normal_btn_url = pricing_btn.data("normal-link");
    pricing_btn.attr("href", normal_btn_url);
  }

  //smart btn
  let TM_Pricing_Switcher_Smart = function ($scope) {
    let pricing_smart_switcher = $('.tm-pricing-smart-switcher, .tm-pricing-plan-switcher');
    if( pricing_smart_switcher.length > 0 ) {
      pricing_smart_switcher.find("[data-pricing-trigger]").on("click", function (e) {
        ...
        let pricing_tables = $self.parents("section").find(".tm-pricing-table");
  ...
  //round, flat btn
  let TM_Pricing_Switcher_Btn = function ($scope) {
    let pricing_btn_switcher = $('.tm-pricing-smart-switcher-button');
    if( pricing_btn_switcher.length > 0 ) {
      pricing_btn_switcher.find("[data-pricing-trigger]").on("click", function (e) {
        let target_id = $(this).data('show');
        let $self = $(e.target);
  ...
  $(".quantity-box .sub").on("click", function () {
	...
	if ($('.dial').length) {
		$('.dial').appear(function () {
			let elm = $(this);
			let color = elm.attr('data-fgColor');
	...
	if ($('.destination-active').length) {
		let swiper = new Swiper(".destination-active", {
			speed:1500,
			loop: true,
			slidesPerView: 1,
			autoplay: {
	...
	if ($('.testimonial-active-home-two').length) {
		let swiper = new Swiper(".testimonial-active-home-two", {
			speed:1500,
			loop: true,
			slidesPerView: 1,
			spaceBetween: 50,
	...
	if($('.scroll-to-fixed-parent').length){
	  let scroll_childs = $('.scroll-to-fixed-child');
	  for (let i = 0, length = scroll_childs.length; i < length; i++) {
	    let scroll_child = $(scroll_childs[i]);
	    scroll_child.scrollToFixed({
	      ...
	      removeOffsets: true,
	      limit: function() {
	        let parent = this.parents('.scroll-to-fixed-parent');
  ...
  /* ---------------------------------------------------------------------- */
  let $onepage_nav = $('.onepage-nav');
  let $sections = $('section');
  let $window = $(window);
  function TM_activateMenuItemOnReach() {
	  if( $onepage_nav.length > 0 ) {
	    let cur_pos = $window.scrollTop() + 2;
	    let nav_height = $onepage_nav.outerHeight();
